...
class Sync1C:
    ...
    def get_all_documents(self, entity_name, filter_posted=True):
        """Универсальный метод загрузки всех документов"""
        params = {"$format": "json"}
        if filter_posted:
    ...
    def get_name_by_key(self, cache, catalog_name, key, name_field='Description'):
        ...
        try:
            r = self.session.get(
                f"{self.base_url}/{catalog_name}(guid'{key}')?$format=json",
                timeout=30
            )
            if r.status_code == 200:
